		...
		add.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				button.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if (!name.getText().trim().isEmpty() && !path.getText().trim().isEmpty()
								&& !user.getText().trim().isEmpty() && !password.getText().trim().isEmpty()) {
							try {
								String result = controller.addDB(name.getText().trim(), path.getText().trim(),
										Integer.parseInt(port.getText().trim()), user.getText().trim(),
										password.getText().trim());
								String str = name.getText().trim() + " " + path.getText().trim() + " "
										+ port.getText().trim() + " " + user.getText().trim() + " "
										+ password.getText().trim() + "\n";
								BufferedWriter writer;
								if (result.equals("Success")) {
									try {
										writer = new BufferedWriter(
												new FileWriter("C:\\soft\\javaDb\\dblist.txt", true));
										writer.append(str);
										writer.close();
				...
				button.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						for (Model model : controller.getModels()) {
							if (model.DBname.equals(name.getText().trim()) && model.DBpath.equals(path.getText().trim())
									&& model.DBport == Integer.parseInt(port.getText().trim())) {
			...
			if (!model.actualDBstatus && !model.actualDBstatus) {
				...
				if (model.deadDate.isEmpty()) {
					...
					try {
						...
						writer.newLine();
						writer.append("Time: " + dtf.format(now) + ", DB: " + model.DBname + " on " + model.getPath()
								+ ":" + model.getPort() + " disabled");
						writer.close();
